    ...
    <nav className="flex items-center justify-between bg-base-100 shadow-sm px-4 md:px-8 py-4">
      ...
      {menuOpen && (
        <div className="absolute top-full left-0 w-full bg-base-100 shadow-sm flex flex-col gap-4 p-4 md:hidden">
          <Link to="/" className="text-base font-normal hover:text-blue-700" onClick={() => setMenuOpen(false)}>
            Home
          </Link>
          <Link to="/signin" className="text-base font-normal hover:text-blue-700" onClick={() => setMenuOpen(false)}>
            Sign In
          </Link>
          <Link
            to="/signup"
